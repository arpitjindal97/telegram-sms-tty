...
public class MySMSBot extends TelegramLongPollingBot
{
    ...
    public void onUpdateReceived(Update update)
    {
        ...
        if (update.hasMessage() && update.getMessage().hasText())
        {
            ...
            send("Processing with amount " + Database.amount, chat_id);

            log.info("Recieved " + message_text);

            message_text = "RC 91" + message_text + " " + Database.amount + " " + Database.pin;

            boolean result = sendSMS.sendOne(Database.dest, message_text);
    ...
    public boolean checkSpecial(String str, long chat_id)
    {
        ...
        {
            Database.amount = builder.substring(builder.indexOf("=") + 1, builder.length());
            send("Amount=" + Database.amount, chat_id);
        }
        else if(str.contains("setPin=") && str.substring("setPin=".length()-1,str.length()).length() > 0)
        {
            Database.pin = builder.substring(builder.indexOf("=") + 1, builder.length());
            send("Pin=" + Database.pin, chat_id);
        }
        else if(str.contains("setDest=") && str.substring("setDest=".length()-1,str.length()).length() > 0)
        {
            Database.dest = builder.substring(builder.indexOf("=") + 1, builder.length());
            send("Dest=" + Database.dest, chat_id);
        }
